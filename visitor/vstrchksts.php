                    ...
                    <div class="row-fluid section">.
                        ...
                        <div class="block">
                            <div class="navbar navbar-inner block-header">
                                <div class="pull-center"><legend><h2>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Visitor's Hostel Accomodation Request Status</h2></legend></div>
                                               ...
                                               <form method="post" class="form-horizontal">
													...
													<div class="control-group">
													  <label class="control-label">Request No<span class="required">*</span></label>
														<div class="controls">
														  <input name="chkststxt" type="text" pattern="{18}" title="Please Enter Valid Request No." class="span6 m-wrap" placeholder="VST-25032017065030" required/>
														</div>
													</div>
												  <div class="controls">
					...
					<!-- DISPLAYING STATUS TABLE START -->			
						...
						if(isset($_POST['chkstatus']))
						{
							$data=1;
								...
								$query=$con->query("SELECT *FROM vstrrequest WHERE vstreqno='$reqno'");
							     if($query->rowCount())
								 {
									 
									/* TO GENERATE FIRST NAME AND LAST NAME FROM THE visitor_details TABLE */
									$query=$con->query("SELECT fname,lname FROM visitor_details WHERE visitorid=(SELECT visitorid FROM vstrrequest WHERE vstreqno='$reqno')");
									while($res=$query->fetch())
										{
											echo $res['fname'] ."	".$res['lname'];
										}
										 echo "</td>";
											/* TO GENERATE REQUEST NO , REQUEST DATE AND REQUEST STATUS FROM vstrrequest TABLE */
											$query=$con->query("SELECT *FROM vstrrequest WHERE vstreqno='$reqno'");
											   while($res=$query->fetch())
											   {
													echo "<td>".$res['vstreqno']."</td>";
													echo "<td>".Date("d-m-Y",strtotime($res['vstrreqdate']))."</td>";
													echo "<td>".$res['vstreqstatus']."</td>";
													if($res['vstreqstatus']=='NEW')
													  echo "<td>"."Request is Not processed yet"."</td>";
												    if($res['vstreqstatus']=='Forwarded')
													  echo "<td>".$res['hostelid']." "."<a href="."display_allotedhstl.php?hostelid=".$res['hostelid']." target="."_blank".">"."Show</a></td>";
												    if($res['vstreqstatus']=='Accepted')
													  echo "<td>".$res['hostelid']." "."<a href="."display_roomallotedhstl.php?reqno=".$reqno." target="."_blank".">"."Show</a></td>";
												  	if($res['vstreqstatus']=='Rejected')
