...
if(isset($_GET['reqno']))
{
	...
	$query =$con->query("SELECT hostelid,stdudent_id FROM stdrequest WHERE requestno='$reqno'");
	$res=$query->fetch();
	$hostelid=$res['hostelid'];
	$stdudent_id=$res['stdudent_id'];
	$query =$con->query("SELECT *FROM hostel_details WHERE hostel_id='$hostelid'");
	$result=$query->fetch();
	$queryselect =$con->query("SELECT room_no FROM room_allotment WHERE  hostel_id='$hostelid' AND allocatee_id='$stdudent_id'");
