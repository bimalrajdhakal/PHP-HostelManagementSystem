...
try{
	...
	$connect->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		...
		{
				$custid=$_POST['custid'];
				$exp_delidate=$_POST['deliverydate'];
				$date=Date("Y-m-d");
				$status="Not Delivered";
				
			 $query =$connect->query("SELECT sum(amount) FROM laundry_order where invoice_no='$invoice_no'");	
              while( $sum=$query->fetch())
			  {
                     $total=$sum['sum(amount)'];
			  }
			$sql=("INSERT INTO laundry_invoice(customer_id,invoice_no,amount,date,exp_delidate,status)
			VALUES (:custid,:invoice_no, :total ,:date, :exp_delidate, :status)");
  ...
  <section class="invoice">
    ...
    <div class="row invoice-info">
      ...
      <div class="col-sm-4 invoice-col">
		...
		<b>Expected Delivery Date:</b> <?php echo $exp_delidate;?><br>
      </div>
      <!-- /.col -->
    </div>
    <!-- /.row -->
        ...
        <div class="table-responsive">
          <table class="table">
			...
			<div class="form-group col-sm-3">
			     <a href=shownewdelivery.php><button type="submit" name="addservice" class="btn btn-info pull-right"><i class="fa fa-backward"></i> Back</button></a>
			  </div>
      </div>
      <!-- /.col -->
    </div>
