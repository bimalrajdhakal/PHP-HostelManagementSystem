	...
	$connect->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		if(isset($_POST['addservice']))
		{
			if($_POST['selectfood']=='Select Food Item...')
			 {
	            echo "<script>alert('Please Select Food Item')</script>";
             }
			else   
			{
				
				$food_id=$_POST['selectfood'];
				$qty=$_POST['quantity'];
				$date=Date("Y-m-d");
				
				$query =$connect->query("SELECT * FROM messentry where foodid='$food_id'");
				 while($res=$query->fetch())
				 {
				    $food_item=$res['fooditem'];
                    $price=$res['price'];
				 }
			$sql=("INSERT INTO mess_order(invoice_no,food_item,price,qty,amount,date)
			VALUES (:invoice_no, :food_item, :price, :qty, :amount ,:date)");
...
<head>
  ...
  <title>Other Staff| Mess Items</title>
  ...
  <div class="content-wrapper">
          ...
          <div class="box box-warning">
			...
			<form method="post" class="form-horizontal">
	            <div class="alert alert-success alert-dismissible <?php if(!isset($serviceinserted)) echo 'hide';?>">
				   ...
				   <h5><i class="icon fa fa-check"></i>Food Item Inserted Sucessfully with Invoice No.: <?php echo $invoice_no;?></h5>
				</div>
			<fieldset>
             <div class="box-body">
				<div class="form-group">
                  <label name="search" class="col-sm-3 control-label">Food Item:<span class="required">*</span></label>
                  <div class="col-sm-3">
					...
					<?php
							...
							$con->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
								$comboquery =$con->query("SELECT foodid,fooditem,price FROM messentry");
   ...
   <!-- Start Table -->
		...
		<?php 
			...
			$con=new PDO ("mysql:host=localhost;dbname=hmsportal","root","");
			$con->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$query =$con->query("SELECT * FROM mess_order where invoice_no='$invoice_no'");
			$i=1;
			...
			while($res=$query->fetch())
			{
				   echo "<tr>";
                   echo "<td>".$i."</td>";
                   echo" <td class="."mailbox-name>".$res['food_item'] ."</td> ";
				   ...
				   echo" <td> <a href=delete.php?trnid=".$res['trn_id']."&invoiceno=$invoice_no> <button type=button class=btn btn-warning ><i class=fa fa-remove></i> Remove </button></a></td>";
				   echo "</tr>";
				   $i++;
							 
			}
                     
             $query =$con->query("SELECT sum(amount) FROM mess_order where invoice_no='$invoice_no'");	
		...
		?>
			  ...
			  <div class="form-group col-sm-3">
			     <a href=proceed.php?invoiceno=<?php echo $invoice_no;?>><button type="submit" name="addservice" class="btn btn-info pull-right"><i class="fa fa-save"></i> Proceed</button></a>
			  </div>
