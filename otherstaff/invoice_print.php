...
try{
	...
	$connect->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		if(isset($_POST['addservice']))
		{
				$custid=$_POST['custid'];
				$date=Date("Y-m-d");
				$bill_cycle=Date("M")."-".Date("Y");
				
			 $query =$connect->query("SELECT sum(amount) FROM mess_order where invoice_no='$invoice_no'");	
              while( $sum=$query->fetch())
			  {
                     $total=$sum['sum(amount)'];
			  }
			$sql=("INSERT INTO mess_invoice(invoice_no,customer_id,amount,date,bill_cycle)
			VALUES (:invoice_no, :custid, :total ,:date,:bill_cycle)");
    ...
    <div class="row">
      <div class="col-xs-12">
        <h2 class="page-header">
          <i class="fa fa-globe"></i> Mess Bill
    ...
    <div class="row invoice-info">
      <div class="col-sm-4 invoice-col">
        ...
        <address>
          ...
          Mess & Hostel Department<br>
          College of Engineering<br>
          Vadakara<br>
          Email: messandhostel@gmail.com
        </address>
      </div>
      <!-- /.col -->
      <div class="col-sm-4 invoice-col">
	  ...
	  <?php 
			...
			$query =$con->query("SELECT * FROM mess_invoice where invoice_no='$invoice_no'");
