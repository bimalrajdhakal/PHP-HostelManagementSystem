...
try{
	...
	$connect->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		if(isset($_POST['addfactor']))
		{
			  ...
			  $query =$connect->query("SELECT * FROM bill_master_table where hostel_id='$hostelid' AND bill_cycle='$bill_cycle'");
			  $res=$query->rowCount();
			  if($res>0)
			  {
				  $error=1;
			  ...
			  {
				  /* GENERATE UNIQUE INVOICE NUMBER FOR EACH STUDENT START */
				  	function createPassword($length)
                        {
						  ...
						  /* GENERATE UNIQUE INVOICE NUMBER FOR EACH STUDENT END */
			   ...
			   foreach ($result as $row)
			   {
				   // fetch records from laundry_invoice table 
					  ...
					  // inserting fetched data to prepare bill for the individual student
                        ...
                        $sinvno=createPassword(11);
				  
				  
				  
				  /* END OF INVOICE GENERATOR FUNCTION */
					  
					  
					  
					  
			$sql=("INSERT INTO bill_master_table(sinvno,student_id,hostel_id,bill_cycle,billing_date,room_rent,water_charge,elec_charge,main_charge,misc_charge,mess_charge,laundry_charge,status)
			VALUES (:sinvno,:student_id, :hostel_id,:bill_cycle, :billing_date ,:room_rent,:water_charge,:elec_charge,:main_charge,:misc_charge,:mess_charge,:laundry_charge,:status)");
