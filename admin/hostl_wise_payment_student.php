    ...
    <section class="content">
	  ...
	  <?php 
		...
		if(isset($_POST['addfactor']))
		{
			 ...
			 $sql=("SELECT * from student_bill_payment_master WHERE hostel_id='$hostelid' and bill_cycle='$bill_cycle'");
			 $query =$con->query($sql);
			 if($query->rowCount()==0)
				 $error=1;
			 else
		...
		?>
		  ...
		  <div class="control-group alert alert-danger text-danger <?php if(!isset($error)) echo 'hide';?>">
                  No data to display , perhaps you entered wrong criteria  or Hostel doesn't exist!
          </div>
